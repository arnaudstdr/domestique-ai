...
def _insert(
    ...
    **extra: float | None,
    ...
    for key, value in extra.items():
        ...
        vals.append(value)
    conn.execute(
        f"INSERT INTO activities ({', '.join(cols)}) "
...
def test_zwift_bucket_matching(tmp_path):
    ...
    _insert(conn, garmin_id=101, date="2026-05-13T10:00:00Z", distance=13700.0, sport_type="VirtualRide")
    conn.commit()
...
def test_duration_anomaly_ignored(tmp_path):
    ...
    conn = _db(tmp_path)
    _insert(conn, strava_id=1, date="2026-05-25T10:00:00Z", distance=35900.0, duration=12300, sport_type="Ride")
    _insert(conn, garmin_id=101, date="2026-05-25T10:00:00Z", distance=35900.0, duration=4800, sport_type="Ride")
    conn.commit()
    assert len(find_duplicate_pairs(conn)) == 1
...
def test_orphans_kept(tmp_path):
    ...
    conn = _db(tmp_path)
    _insert(conn, strava_id=1, date="2026-01-02T10:00:00Z", distance=14000.0, sport_type="VirtualRide")
...
def test_internal_strava_duplicate(tmp_path):
    conn = _db(tmp_path)
    _insert(conn, strava_id=1, date="2026-05-20T10:00:00Z", distance=20700.0, duration=3840, sport_type="Ride")
    _insert(conn, strava_id=2, date="2026-05-20T10:00:00Z", distance=20700.0, duration=3840, sport_type="Ride")
    conn.commit()
...
def test_internal_duplicate_matched_as_pair_not_double_counted(tmp_path):
    ...
    _insert(conn, strava_id=1, date="2026-05-20T10:00:00Z", distance=20700.0, duration=3840, sport_type="Ride")
    _insert(conn, strava_id=2, date="2026-05-20T10:00:00Z", distance=20700.0, duration=3840, sport_type="Ride")
    _insert(conn, garmin_id=101, date="2026-05-20T10:00:00Z", distance=20704.4, duration=3602, sport_type="VirtualRide")
    conn.commit()
    pairs = find_duplicate_pairs(conn)
    ...
    try:
        ...
        assert conn.execute(
            "SELECT COUNT(*) FROM activities WHERE strava_id IS NOT NULL"
        ).fetchone()[0] == 0
    finally:
        conn.close()
...
def test_dedupe_merges_and_deletes(tmp_path):
    conn = _db(tmp_path)
    _insert(conn, strava_id=1, date="2026-05-06T10:00:00Z", distance=16300.0, sport_type="Ride", avg_power=250.0)
    _insert(conn, garmin_id=101, date="2026-05-06T10:00:00Z", distance=16300.0, sport_type="Ride", avg_power=None)
    _insert(conn, strava_id=2, date="2026-05-07T10:00:00Z", distance=20700.0, duration=3840, sport_type="Ride")
    _insert(conn, strava_id=3, date="2026-05-07T10:00:00Z", distance=20700.0, duration=3840, sport_type="Ride")
    _insert(conn, strava_id=4, date="2026-01-02T10:00:00Z", distance=14000.0, sport_type="VirtualRide")
    conn.commit()
    ...
    try:
        ...
        g = conn.execute(
            "SELECT avg_power FROM activities WHERE garmin_id = 101"
        ).fetchone()
        assert g[0] == 250.0
        assert conn.execute(
            "SELECT COUNT(*) FROM activities WHERE strava_id = 1"
        ).fetchone()[0] == 0
        assert conn.execute(
            "SELECT COUNT(*) FROM activities WHERE strava_id = 4"
        ).fetchone()[0] == 1
    finally:
        conn.close()
